function MakeBlobThreadMovie(tiffStackOrginalFile,stack,blobs,blobThreads,validZs)
    ...
    disp('MakeBlobThreadMovie> making blob movie.');
    tic
    
    %set up output movie directory and files
    movieOutName=[tiffStackOrginalFile(1:end-4)  '-BlobThreadMovie.mp4'];
    ...
    for z=validZs
            ...
            for j=1:blobThreads.n


                if blobThreads.z(j)==z
                    
                    plot(blobThreads.x0(j),blobThreads.y0(j),'co','MarkerSize',10);
